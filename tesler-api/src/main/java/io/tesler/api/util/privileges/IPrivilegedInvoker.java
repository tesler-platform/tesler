...
public interface IPrivilegedInvoker<T, E extends Throwable> {
	...
	Subject getSubject(String securityAlias);

	T runAs(Invoker<T, E> action, Subject... subjects) throws E;

	T runAs(PrivilegedAction<T> action, String alias);

	T runAs(PrivilegedAction<T> action, Subject... subjects);
